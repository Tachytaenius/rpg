...
local cpml = require("lib.cpml") -- Line changed by Tachytaenius
local TransposeMatrix, InvertMatrix, CrossProduct, UnitVectorOf, VectorLength, ScaleVerts, MoveVerts -- Line added by Tachytaenius
local engine = {}
...
function engine.newScene(renderWidth,renderHeight)
    ...
    scene.render = function (self, drawArg, bgr, bgg, bgb, bga)
		bgr, bgg, bgb, bga = bgr or 0, bgg or 0, bgb or 0, bga or 0
        love.graphics.setColor(1,1,1)
        love.graphics.setCanvas({self.threeCanvas, depth=true})
        love.graphics.clear(bgr, bgg, bgb, bga) -- Background colour changing added by Tachytaenius
		...
		print(Camera.perspective * TransposeMatrix(camTransform))
		-- Lines commented by Tachytaenius
        ...
        for i=1, #self.modelList do
            ...
            if model ~= nil and model.visible and #model.verts > 0 then
				-- Variable name casing changed by Tachytaenius
                self.threeShader:send("modelMatrix", model.transform)
                self.threeShader:send("modelMatrixInverse", TransposeMatrix(InvertMatrix(model.transform)))

                love.graphics.setWireframe(model.wireframe)
                if model.culling then
                    love.graphics.setMeshCullMode(model.culling) -- Line changed by Tachytaenius
                end
